<template>
  <div class="pe-6">
    ...
    <ol v-if="status === 'success'" class="relative border-s border-gray-500">
      <li class="mb-10 ms-4" v-for="blog in latestBlogs">
        <NuxtLink :to="`/blog/${blog.documentId}`">
          ...
          ></div>
          <span class="mb-1 text-sm font-normal leading-none text-gray-400">
...
const latestBlogs = ref<Blog[]>(data.value.data as Blog[]);
</script>
